...
class SlamlBenchmarks
  ...
  def init_compiled_benches
    ...
    bench('(1) slaml pretty') { context.run_slaml_pretty }
    bench('(1) slaml ugly')   { context.run_slaml_ugly }
  end
  ...
  def init_tilt_benches
    ...
    bench('(2) slaml pretty') { tilt_slaml_pretty.render(context) }
    bench('(2) slaml ugly')   { tilt_slaml_ugly.render(context) }
  end
  ...
  def init_parsing_benches
    ...
    bench('(3) slaml pretty') { Slaml::Template.new(:pretty => true) { @haml_code }.render(context) }
    bench('(3) slaml ugly')   { Slaml::Template.new(:pretty => false) { @haml_code }.render(context) }
  end
...
(3) Parsing benchmark. Template is parsed every time.
    ...
    and Slaml is not optimized for it. Activate this benchmark with 'rake bench slow=1'.
...
SlamlBenchmarks.new(ENV['slow'], ENV['iterations']).run
